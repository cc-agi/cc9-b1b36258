...
check("version consistency @ 0.4.21", () => {
  const versionTs = readFileSync(resolve(ROOT, "src/lib/mcp/version.ts"), "utf8");
  const mustMatch = {
    MCP_CODE_VERSION: "0.4.21",
    MCP_MANIFEST_VERSION: "0.4.21",
    MIN_HELPER_VERSION: "0.4.21",
  };
  ...
  if (manifestVersion !== "0.4.21") {
    throw new Error(
      `.lovable/mcp/manifest.json server.version=${manifestVersion} (expected 0.4.21)`,
    );
  }
  const helperPkg = JSON.parse(readFileSync(resolve(ROOT, "helper/package.json"), "utf8"));
  if (helperPkg.version !== "0.4.21") {
    throw new Error(`helper/package.json version=${helperPkg.version} (expected 0.4.21)`);
  }
  const indexMjs = readFileSync(resolve(ROOT, "helper/src/index.mjs"), "utf8");
  const im = indexMjs.match(/VERSION\s*=\s*"([^"]+)"/);
  if (!im || im[1] !== "0.4.21") {
    throw new Error(`helper/src/index.mjs VERSION=${im?.[1]} (expected 0.4.21)`);
  }
  const pairMjs = readFileSync(resolve(ROOT, "helper/src/pair.mjs"), "utf8");
  const pm = pairMjs.match(/VERSION\s*=\s*"([^"]+)"/);
  if (!pm || pm[1] !== "0.4.21") {
    throw new Error(`helper/src/pair.mjs VERSION=${pm?.[1]} (expected 0.4.21)`);
  }
});
...
check("0.4.20 SendInput unicode + verified type + dynamic helper version", () => {
  ...
  const status = readFileSync(resolve(ROOT, "helper/status-helper.ps1"), "utf8");
  if (/\$HELPER_VERSION\s*=\s*"0\.\d+\.\d+"/.test(status)) {
    throw new Error("status-helper.ps1 still hardcodes HELPER_VERSION (must read package.json)");
  }
  if (!status.includes("package.json") || !status.includes("ConvertFrom-Json")) {
