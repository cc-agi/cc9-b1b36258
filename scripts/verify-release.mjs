...
check("version consistency @ 0.4.6", () => {
  const versionTs = readFileSync(resolve(ROOT, "src/lib/mcp/version.ts"), "utf8");
  const mustMatch = {
    MCP_CODE_VERSION: "0.4.6",
    MCP_MANIFEST_VERSION: "0.4.6",
    MIN_HELPER_VERSION: "0.4.6",
  };
  ...
  if (manifestVersion !== "0.4.6") {
    throw new Error(
      `.lovable/mcp/manifest.json server.version=${manifestVersion} (expected 0.4.6)`,
    );
  }
  const helperPkg = JSON.parse(readFileSync(resolve(ROOT, "helper/package.json"), "utf8"));
  if (helperPkg.version !== "0.4.6") {
    throw new Error(`helper/package.json version=${helperPkg.version} (expected 0.4.6)`);
  }
  const indexMjs = readFileSync(resolve(ROOT, "helper/src/index.mjs"), "utf8");
  const im = indexMjs.match(/VERSION\s*=\s*"([^"]+)"/);
  if (!im || im[1] !== "0.4.6") {
    throw new Error(`helper/src/index.mjs VERSION=${im?.[1]} (expected 0.4.6)`);
  }
  const pairMjs = readFileSync(resolve(ROOT, "helper/src/pair.mjs"), "utf8");
  const pm = pairMjs.match(/VERSION\s*=\s*"([^"]+)"/);
  if (!pm || pm[1] !== "0.4.6") {
    throw new Error(`helper/src/pair.mjs VERSION=${pm?.[1]} (expected 0.4.6)`);
  }
});
...
check("0.4.6 hotfix: qualified ACL principal, bounded heartbeat, envelope journal", () => {
  const ps = readFileSync(resolve(ROOT, "helper/desktop-operator.ps1"), "utf8");
...
// The 0.4.6 field regression proved that a single `inputSchema: null` (from
